...
export default class MissingPermissionsListener extends Listener {
	...
	public exec(msg: Message, _: Command, type: any, missing: any): void | Promise<Message | Message[] | void> {
		if (missing === 'notOwner') {
			return msg.channel.send('This command is reserved for the server owner.').catch(() => {});
		}
	...
	public missingPermissions(channel: TextChannel, user: User, permissions: any): string {
		const missingPerms = channel.permissionsFor(user)!.missing(permissions)
			.map((str: any) => {
				...
				return `\`${str.replace(/_/g, ' ').toLowerCase().replace(/\b(\w)/g, (char: any): string => char.toUpperCase())}\``;
			});
