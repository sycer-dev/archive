...
export default class PrefixCommand extends Command {
	...
	public async exec(msg: Message, { prefix }: { prefix: string }): Promise<Message | Message[] | void> {
		if (!prefix) {
			const prefix = this.client.settings.cache.clients.get(this.client.user!.id)!.prefix;
			return msg.util?.reply(`the current prefix is \`${prefix}\`.`);
		}

		await this.client.settings.set('client', { id: this.client.user!.id }, { prefix: prefix });
