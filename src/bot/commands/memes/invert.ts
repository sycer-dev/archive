...
export default class InvertCommand extends ToolCommand {
	public constructor() {
		super('invert', {
			channel: 'guild',
			category: 'memes',
			aliases: ['invert'],
			description: {
				content: "invert someone's avatar",
			},
			cooldown: 3,
	...
	public async exec(msg: Message, { user }: { user: User }): Promise<Message | Message[] | void> {
		try {
			const meme = await this.client.memeHandler.makeMeme('invert', {
				avatar1: user.displayAvatarURL({ size: 2048, format: 'png' }),
			});
			return msg.util?.reply({ files: [{ name: 'invert.png', attachment: meme }] });
		} catch (err) {
