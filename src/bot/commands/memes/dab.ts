...
export default class DabCommand extends ToolCommand {
	public constructor() {
		super('dab', {
			channel: 'guild',
			category: 'memes',
	...
	public async exec(msg: Message, { user }: { user: User }): Promise<Message | Message[] | void> {
		try {
			const meme = await this.client.memeHandler.makeMeme('dab', {
				avatar1: user.displayAvatarURL({ size: 2048, format: 'png' }),
			});
			return msg.util?.reply({ files: [{ name: 'dab.png', attachment: meme }] });
		} catch (err) {
